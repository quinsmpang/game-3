...
strFrame='<style>';
strFrame+='INPUT.button{BORDER-RIGHT: #1c4b7d 1px solid;BORDER-TOP: #1c4b7d 1px solid;BORDER-LEFT: #1c4b7d 1px solid;';
strFrame+='BORDER-BOTTOM: #1c4b7d 1px solid;BACKGROUND-COLOR: #fff8ec;font-family:宋体;}';
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 width=142 height=160 bordercolor=#1c4b7d bgcolor=#1c4b7d Author="wayx">';
...
strFrame+='      <tr align=center Author="wayx"><td width=16 align=center bgcolor=#1c4b7d style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='        onclick="parent.meizzPrevM()" title="向前翻 1 月" Author=meizz><b Author=meizz>&lt;</b>';
strFrame+='        </td><td width=60 align=center style="font-size:12px;cursor:default" Author=meizz ';
strFrame+='onmouseover="style.backgroundColor=\'#6fa8e5\'" onmouseout="style.backgroundColor=\'white\'" ';
...
strFrame+='<td width=48 align=center style="font-size:12px;cursor:default" Author=meizz onmouseover="style.backgroundColor=\'#6fa8e5\'" ';
strFrame+=' onmouseout="style.backgroundColor=\'white\'" onclick="parent.tmpSelectMonthInnerHTML(this.innerText.length==3?this.innerText.substring(0,1):this.innerText.substring(0,2))"';
strFrame+='        title="点击这里选择月份"><span id=meizzMonthHead Author=meizz></span></td>';
strFrame+='        <td width=16 bgcolor=#1c4b7d align=center style="font-size:12px;cursor: hand;color: #ffffff" ';
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 bgcolor=#1c4b7d ' + (bMoveable? 'onmousedown="DragStart()" onmouseup="DragEnd()"':'');
strFrame+=' BORDERCOLORLIGHT=#1c4b7d BORDERCOLORDARK=#FFFFFF width=140 height=20 Author="wayx" style="cursor:' + (bMoveable ? 'move':'default') + '">';
...
strFrame+='    <table border=1 cellspacing=2 cellpadding=0 BORDERCOLORLIGHT=#1c4b7d BORDERCOLORDARK=#FFFFFF bgcolor=#fff8ec width=140 height=120 Author="wayx">';
...
var obname=""; //赵敏修改 选择日期后是否需要调用其它函数的名称
var useTime=false;

function setday_item(tt,obj,ob_n) //赵敏修改后的主调函数
{
  if(ob_n!=""){obname=ob_n;}
  setday(tt,obj);
...
{	
	...
	if(r!=null){
		...
		if(d.getFullYear()==r[1] && d.getMonth()==r[2] && d.getDate()==r[3]){
			...
			myup=meizzSetDay(r[1],r[2]+1);
	}
	else{
		outDate="";
		myup=meizzSetDay(new Date().getFullYear(), new Date().getMonth() + 1);
	}
	dads.display = '';
      ...
      { 
		...
		if(i<firstday)		//上个月的部分
		{
			...
			else
			{
				...
				meizzWDay[i]==outDate.getDate())? "#6fe0e5" :
				(((mm==1?yy-1:yy) == new Date().getFullYear() && (mm==1?12:mm-1) == new Date().getMonth()+1 && 
				meizzWDay[i] == new Date().getDate()) ? "#6fa8e5":"#e0e0e0");
				//将选中的日期显示为凹下去
				if((mm==1?yy-1:yy)==outDate.getFullYear() && (mm==1?12:mm-1)== outDate.getMonth() + 1 && 
				meizzWDay[i]==outDate.getDate())
				{
					da.borderColorLight="#FFFFFF";
					da.borderColorDark="#1c4b7d";
				}
			}
		}
		...
		else		//本月的部分
		{
			...
			else
			{
				da.style.backgroundColor =(yy==outDate.getFullYear() && mm== outDate.getMonth() + 1 && meizzWDay[i]==outDate.getDate())?
					"#6fe0e5":((yy == new Date().getFullYear() && mm == new Date().getMonth()+1 && meizzWDay[i] == new Date().getDate())?
					"#6fa8e5":"#e0e0e0");
				//将选中的日期显示为凹下去
...
function meizzDayClick(n,ex)  //点击显示框选取日期，主输入函数*************
{
  ...
  if (outObject)
  {
    if (!n) {//outObject.value=""; 
      return;}
    if ( n < 10){n = "0" + n;}
    outObject.value= yy + "-" + mm + "-" + n  + (useTime?" 00:00:00":""); //注：在这里你可以输出改成你想要的格式
    closeLayer(); 
  }
  else {closeLayer(); alert("您所要输出的控件对象并不存在！");}
  if(obname!=""){eval(obname);}
}
function setObname(nn){
  obname=nn;
}