 ...
 * @author wkc
 */
public class PlaJJShopBAC {
	public static final String tab_jj_shop = "tab_jj_shop";
	public static final String tab_jj_shop_stone = "tab_jj_shop_stone";
	public static final String tab_jj_shop_time = "tab_jj_shop_time";
	public static final String tab_jj_shop_refresh = "tab_jj_shop_refresh";
	...
	public void init(DBHelper dbHelper, int playerid, Object... parm) throws Exception {
		...
		sqlStr.add("item3", dataarr.optJSONArray(0).toString());
		sqlStr.add("buy3", dataarr.optJSONArray(1).toString());
		sqlStr.addDateTime("refreshtime3", MyTools.getTimeStr(getNextRefreshTime()));
		sqlStr.add("times3", 0);
	...
	public ReturnValue getShopData(int playerid){
		...
		try {
			...
			if(!plaJJRs.exist()){
				BACException.throwInstance("竞技商店尚未开放");
			}
			dbHelper.openConnection();
			JSONArray returnarr = new JSONArray();
			returnarr.add(plaJJRs.getInt("times3"));
			if(MyTools.checkSysTimeBeyondSqlDate(plaJJRs.getTime("refreshtime3"))){
				GameLog gl = GameLog.getInst(playerid, GameServlet.ACT_JJ_SHOP_GETDATA);
				JSONArray refresharr = refresh(dbHelper, playerid, true, gl);
				MyTools.combJsonarr(returnarr, refresharr);
				gl.save();
			} else {
				returnarr.add(new JSONArray(plaJJRs.getString("item3")));
				returnarr.add(new JSONArray(plaJJRs.getString("buy3")));
				returnarr.add(plaJJRs.getTime("refreshtime3"));
			}
	...
	public ReturnValue buy(int playerid, int index){
		...
		try {
			DBPaRs plaJJRs = PlaShopBAC.getInstance().getDataRs(playerid);
			JSONArray buyarr = new JSONArray(plaJJRs.getString("buy3"));
			if(buyarr.size() == 0){
				BACException.throwInstance("竞技商店尚未开放");
			}
			...
			dbHelper.openConnection();
			GameLog gl = GameLog.getInst(playerid, GameServlet.ACT_JJ_SHOP_BUY);
			JSONArray itemarr = (new JSONArray(plaJJRs.getString("item3"))).optJSONArray(index);
			int need = Tools.splitStrToIntArr(itemarr.optString(1), ",")[1];
			PlaRoleBAC.getInstance().subValue(dbHelper, playerid, "jjccoin", need, gl, "竞技币");
			JSONArray awardarr = AwardBAC.getInstance().getAward(dbHelper, playerid, itemarr.optString(0), ItemBAC.SHORTCUT_MAIL, 39, gl);
			buyarr.put(index, 1);
			SqlString sqlStr = new SqlString();
			sqlStr.add("buy3", buyarr.toString());
			PlaShopBAC.getInstance().update(dbHelper, playerid, sqlStr);
	...
	public ReturnValue refreshShop(int playerid){
		...
		try {
			...
			int times = plaJJRs.getInt("times3");
			int need = 0;
			DBPsRs conListRs  = DBPool.getInst().pQueryS(tab_jj_shop_refresh);
	...
	public JSONArray refresh(DBHelper dbHelper, int playerid, boolean refreshTime, GameLog gl) throws Exception {
		...
		SqlString sqlStr = new SqlString();
		sqlStr.add("item3", itemarr.toString());
		sqlStr.add("buy3", buyarr.toString());
		if(refreshTime){
			sqlStr.addDateTime("refreshtime3", MyTools.getTimeStr(nexttime));
	...
	public JSONArray refreshItem(int playerid) throws Exception{
		...
		//魂石库
		DBPsRs stoneListRs  = DBPool.getInst().pQueryS(tab_jj_shop_stone);
		...
		DBPaRs shopListRs = DBPool.getInst().pQueryA(tab_jj_shop, "begin<="+plv+" and end>="+plv);
		...
		for(int j = 0; j < amount; j++){
			...
			Object[] enter = ItemBAC.getInstance().enterItem(split, "jjcoinodds");
			...
			if(priceStr.equals("0")){
				...
				priceSb.append("10,");
				priceSb.append(((DBPsRs)enter[1]).getInt("jjcoinprice"));
	...
	public long getNextRefreshTime() throws Exception{
		return ShopData.getNextRefreshTime(ShopData.time_jj);
	}
	
	
	//--------静态区--------
	
	private static PlaJJShopBAC instance = new PlaJJShopBAC();
	
	public static PlaJJShopBAC getInstance(){
		return instance;
	}
