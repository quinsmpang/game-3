...
{
	if(chooseValue==null || chooseValue=="null")return;
	var haveFound=false;
	for(i=0;selectObj.options!=null && i<selectObj.options.length;i++)
	{
...
{		
	...
	{
 		//if(eleArr[i].getAttribute("autoInput")!=null && eleArr[i].getAttribute("autoInput")=="false")continue;
		//if(eleArr[i].autoInput!=null && eleArr[i].autoInput=="false")continue;
		//alert(eleArr[i].name+" tag="+eleArr[i].tagName);
		//自动选择下拉菜单
 		if(eleArr[i].tagName=="SELECT")
		{
			selectValue=allValue[eleArr[i].name];
			//alert("selectValue="+selectValue);
			//alert(selectValue!=null);
			if(selectValue!=null)
			{
			/*
				alert(eval("document.forms[0]."+eleArr[i].name).item.length);
				if(eval("document.forms[0]."+eleArr[i].name).length!=0)
				{ //若该名字的对象有多个
					strArr="";
					if(selectValue!=null)
					{
					...
					for(j=0;objArr!=null && j<objArr.length;j++)
					{
						if(eleArr[i]==objArr.item(j))
						{   //比较对象数组中每个对象和当前对象是否相同
							if(strArr[j]!=null)
							{
								if(strArr[j]=="_")
								{
		...
		if(eleArr[i].tagName=="INPUT" && (eleArr[i].type=="text" || eleArr[i].type=="hidden" || eleArr[i].type=="password"))
		{
			...
			{
				/*
				if((count=eval("document.forms[0]."+eleArr[i].name).length)!=null)
				{ //若该名字的对象有多个	
					strArr="";
					if(inputValue!=null)
					{
...
if(dateValue.indexOf("：")!=-1)
{
	alert("时间格式不能使用中文冒号：请用英文冒号:");
	return false;
}

//2013-1-1 4:4:4
var firstSplitIndex=dateValue.indexOf(splitStr);
var nianStr=dateValue.substring(0,firstSplitIndex);
if(nianStr.length!=4 || isNaN(nianStr))
{
	alert("年格式不对");
	return false;
}
var secondSplitIndex=dateValue.indexOf(splitStr,firstSplitIndex+1);
var yueStr=dateValue.substring(firstSplitIndex+1,secondSplitIndex);
//alert(yueStr)
if((yueStr.length!=1 && yueStr.length!=2) || isNaN(yueStr))
{
	alert("月份格式不对");
	return false;	
}

var blankIndex=dateValue.indexOf(" ",secondSplitIndex+1);
var dayStr=0;
if(blankIndex!=-1)
{
	dayStr=dateValue.substring(secondSplitIndex+1,blankIndex);
}
else
{
	dayStr=dateValue.substring(secondSplitIndex+1);
}
//alert(dayStr);
if((dayStr.length!=1 && dayStr.length!=2) || isNaN(dayStr))
{	
	alert("日格式不对");
	return false;
}
...
{	
	...
	var result = pattern.exec(dateValue); // 用正则表达式验证 
	if(!result){
		return false;
	}else
	{
