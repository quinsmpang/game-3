...
if(disp!=null){
   ...
   if(liobj.id!=null && liobj.id!=""){
   	eval("parent.menu."+liobj.id+"=''");  //将节点状态保存到外部框架页面中
   	}
  }else
  {
   ulobj.style.display="none";
   imgobj.src="../images/icon_add.gif";
   if(liobj.id!=null && liobj.id!=""){
   	eval("parent.menu."+liobj.id+"='none'");  //将节点状态保存到外部框架页面中
   	}
   }
}
...
for(var i=0;i<eleArr.length;i++){
	if(eleArr[i].tagName=="LI"){
		if(eleArr[i].id!=null && eleArr[i].id!=""){
			if(eval("parent.menu."+eleArr[i].id)!=null)
				{
				showhide(eleArr[i],eval("parent.menu."+eleArr[i].id))
				}
